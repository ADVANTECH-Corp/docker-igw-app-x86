...
var restAPI = require('./restfulApi.js');


//  intfNow : Network  (Ethernet/0007xxxx)
//    devNow: SenHub AgentID (001000ec6f0de02)
//  topicNow: Category ( Info or SenData ...)
// attribNow: Attribute ( Name or CO2 ...)
...
module.exports = function(RED) {
    ...
    function VerifyRestOutput(error, stdout, stderr) {
        ...
        if (JsonObj !== null &&
            ...
            JsonObj.hasOwnProperty('Result')) 
        {
            ResultJsonStr = JSON.stringify(JsonObj);
            return true;
        } else {
            ErrorStr = null;
            return false;
    ...
    function WsnSenhubAdv(config) {
        ...
        this.read = function(msgIn) {
            ...
            if( node.operNow === 'None' || node.operNow == '0' || node.operNow === '' ||
                node.intfNow === 'None' || node.intfNow == '0' || node.intfNow === '' ||
                node.devNow === 'None' || node.devNow == '0' || node.devNow === '' ||
                node.topicNow === 'None' || node.topicNow == '0' || node.topicNow === "" ||
                node.attribNow === 'None' || node.attribNow == '0' || node.attribNow === '' ) {
                node.msg.payload = '{"StatusCode":400,"Result":{"sv":"Please select all option"}';
                node.send(node.msg);
                return;
            }   
            ...
            //rest = '"' +  node.devNow + '/SenHub/' + node.topicNow + '/' + node.attribNow + '"'; // apimux
            // Next version support get in SenHub or Category
            /*
            if(node.topicNow === 'None' || node.topicNow == '0')
                rest = node.devNow + '/SenHub';                
            else if ( node.attribNow === 'None' || node.attribNow == '0')
                rest = node.devNow + '/SenHub/' + node.topicNow;
            else
                rest = node.devNow + '/SenHub/' + node.topicNow + '/' + node.attribNow;
            */
            ...
            if (node.operNow === "Set") {
                ...
                restAPI.wsnRestApi(API_PROGRAM + ' POST ' + rest + ' ' + setValueJson, function(error, stdout, stderr) {
                ...
                restAPI.wsnRestApi(API_PROGRAM + ' GET ' + rest, function(error, stdout, stderr) {
                    var isValidResult = VerifyRestOutput(error, stdout, stderr);
                    //var msg = {};
    ...
    // To query "Network" Type => WSN (00170d000030)
    RED.httpAdmin.get("/WSN_SnGetIntf/:id", function(req,res) { // id => GET or SET
        //console.log("In WSN_SnGetIntf !!");
        var restGetIntf = "IoTGW";

        restAPI.wsnRestApi(API_PROGRAM + ' GET ' + restGetIntf, function(error, stdout, stderr) {
            if (res.headersSent) {
                return;
            }
    ...
    // To query "SenHubList" of this Network Interface ex: 000100E9658AE
    RED.httpAdmin.get("/WSN_SnGetDev/:id/:confIntf", function(req,res) { // id => GET or SET, confIntf => Network
        ...
        restAPI.wsnRestApi(API_PROGRAM + ' GET ' + restGetDev, function(error, stdout, stderr) {
            if (res.headersSent) {
                return;
            }
    ...
    // To query "Category" of SenHub from Capability ex: SenData
    RED.httpAdmin.get("/WSN_SnGetTopic/:id/:confIntf/:confDev", function(req,res) { // id => GET or SET, confIntf => Network, confDev => SenHubID
        //console.log("In WSN_SnGetTopic !!");
        var confIntf = req.params.confIntf;        
        var confDev = req.params.confDev;
        //console.log("inf "+confIntf +'dev '+confDev);

        if (confIntf === 'undefined' || confIntf === 'null' ||  confIntf === 'None' || 
            confDev === 'undefined' || confDev === 'null' || confDev === 'None' ) 
        {
        ...
        restAPI.wsnRestApi(API_PROGRAM + ' GET ' + restGetTopic, function(error, stdout, stderr) {
            if (res.headersSent) {
                return;
            }
    ...
    // To query "Attribute" of Category
    RED.httpAdmin.get("/WSN_SnGetAttrib/:id/:confOper/:confIntf/:confDev/:confTopic", function(req,res) { // id => GET or SET, confIntf => Network, confDev => SenHubID, confTopic => Category
        ...
        restAPI.wsnRestApi(API_PROGRAM + ' GET ' + restGetAttrib, function(error, stdout, stderr) {
            if (res.headersSent) {
                return;
            }
